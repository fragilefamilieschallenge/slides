...
\date{June 2, 2017\\UCLA\\\begin{footnotesize}California Center for Population Research and Center for Social Statistics\end{footnotesize}\\\vfill
...
\begin{tikzpicture}
  ...
  \node (Ut) at (0, 2) {$U_T$};
  \node (X) at (-4, 0) {$X$};
  \draw[->, >=stealth, thick, line width = 6pt, blue] (T) -- (Y);
  \draw[->, >=stealth, thick] (X) -- (T);
  \draw[->, >=stealth, thick] (X) to[bend right]  (Y);
  \draw[->, >=stealth, thick] (Ut) --  (T);
  %\draw[->, >=stealth, thick, dashed] (Ut) --  (Y);
...
\begin{tikzpicture}
  ...
  \draw[->, >=stealth, thick] (X) -- (T);
  \draw[->, >=stealth, thick] (X) to[bend right]  (Y);
  \draw[->, >=stealth, blue, line width = 6pt] (Ut) --  (T);
\end{tikzpicture}
\end{center}
